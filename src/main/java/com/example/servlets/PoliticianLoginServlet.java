...
public class PoliticianLoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection connection = DBUtil.getConnection()) {
            String sql = "SELECT * FROM politicians WHERE email = ? AND password = MD5(?)";
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    if (resultSet.next()) {
                        // Login successful, redirect to the politician dashboard
                        response.sendRedirect("politician-home.html");
                    } else {
